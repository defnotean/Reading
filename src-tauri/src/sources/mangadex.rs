...
use crate::sources::{BrowseList, ChapterContent, Source, TitleDetail, TitleSummary};
...
impl Source for MangaDex {
    ...
    async fn browse(&self, list: BrowseList, page: u32) -> AppResult<Vec<TitleSummary>> {
        ...
        match &list {
            ...
            BrowseList::Genre(name) => {
                if let Some(uuid) = tag_uuid_for(name) {
                    query.push_str(&format!(
                        "&order%5BfollowedCount%5D=desc&includedTags%5B%5D={uuid}"
                    ));
                } else {
            ...
            BrowseList::Lang(code) => {
                query.push_str(&format!(
                    "&order%5BfollowedCount%5D=desc&originalLanguage%5B%5D={code}"
                ));
            }
        }
    ...
    async fn title(&self, id: &str) -> AppResult<TitleDetail> {
        ...
        let feed_body = crate::http::client()
            .get(&feed_url)
            .send()
            .await?
            .text()
...
pub mod parse {
    ...
    pub fn feed_response(body: &str) -> AppResult<Vec<ChapterSummary>> {
        ...
        let data = v
            ...
            .and_then(Value::as_array)
            .ok_or_else(|| AppError::Parse("missing data array".into()))?;
        let mut chapters: Vec<ChapterSummary> = data
            .iter()
            .filter_map(|item| {
    ...
    pub fn at_home_response(body: &str, quality: &str) -> AppResult<Vec<PageImage>> {
        ...
        let key = if quality == "data-saver" {
            "dataSaver"
        } else {
            "data"
        };
        let segment = if quality == "data-saver" {
            "data-saver"
        } else {
            "data"
        };
