...
Local Android tooling is installed, `android:init` succeeds, and Windows Developer Mode is enabled for the user account. The official `pnpm.cmd tauri android build --ci --apk` path now completes through Tauri's symlink step and produces the universal release APK; `android:build:windows-copy` remains as a guarded fallback for Windows machines without symlink rights. Emulator smoke on the `medium_phone` AVD has verified install, launch, MangaDex browse content, reader open, Settings navigation, and Library navigation.
